...
export default async function HomePage() {
  ...
  // Indexed on the time-weighted unit value, not the net asset value: a
  // subscription into the fund raises the NAV without being a return.
  const indexed = navSeries.length
    ? navSeries.map((p) => ({
        date: p.date,
        portfolio: (p.unitValue / navSeries[0].unitValue) * 100,
  ...
  const layout = parseJson<{ hidden: string[]; order: string[] }>(workspace?.layout, { hidden: [], order: [] });

  const portfolioSection = {
    id: 'portfolio',
    label: 'Portfolio',
    empty: portfolio ? undefined : true,
    node: (
      <>
        <Panel padded={false}>
          <div className="p-4">
                ...
                {indexed.length > 1 ? (
                  <div className="mt-3">
                    <LineSeriesChart
                      ...
                      subtitle="Unit value indexed to 100 at inception"
  ...
  const watchlistSection = {
    id: 'watchlist',
    label: 'Watchlist',
    empty: data.watchRows.length === 0 || undefined,
    node: (
      <>
        <Panel padded={false}>
  ...
  const insightsSection = {
    id: 'insights',
    label: 'Insights',
    empty: data.insights.length === 0 || undefined,
    node: (
      <>
        <Panel>
          <PanelHeader
  ...
  const thesisSection = {
    id: 'thesis',
    label: 'Thesis health',
    empty: data.thesisHealth.length === 0 || undefined,
    node: (
      <>
        <Panel>
  ...
  const catalystsSection = {
    ...
    empty: data.upcomingCatalysts.length === 0 || undefined,
    node: (
      <>
        <Panel>
  ...
  const researchSection = {
    ...
    empty: data.recentNotes.length === 0 || undefined,
    node: (
      <>
        <Panel>
  ...
  const targetsSection = {
    id: 'targets',
    label: 'Target price changes',
    empty: data.recentTargetChanges.length === 0 || undefined,
    node: (
      <>
  ...
  const earningsSection = {
    ...
    empty: data.recentEarnings.length === 0 || undefined,
    node: (
      <>
  ...
  return (
    <>
      ...
      <DashboardLayout
        main={[portfolioSection, watchlistSection]}
        rail={[insightsSection, thesisSection, catalystsSection, researchSection, targetsSection, earningsSection]}
